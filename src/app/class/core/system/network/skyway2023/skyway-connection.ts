...
// 新SkyWay（@skyway-sdk/core）を使った接続クラス（元祖v1.17.0より移植）
export class SkyWayConnection implements Connection {
  ...
  connect(peer: IPeerContext): boolean {
    if (!this.peer.isRoom) {
      ...
      if (this.callback.onError) this.callback.onError(this.peerId, errorType, errorMessage, {});
      return false;
    }
  ...
  private async openSkyWay(peer: IPeerContext) {
    ...
    this.skyWay.onFatalError = (peer, errorType, errorMessage, errorObject) => {
      ...
      if (this.callback.onError) this.callback.onError(this.peerId, errorType, errorMessage, errorObject);
    };
  ...
  private connectStream(stream: SkyWayDataStream) {
    ...
    stream.on('stats', async () => {
      // 統計情報は skyway-data-stream 内で直接処理される
    });
  ...
  private onUpdateUserIds(stream: SkyWayDataStream, userIds: string[]) {
    ...
    userIds.forEach(userId => {
      ...
      let s = this.streams.find(peer.peerId);
      if (s && s.peer.userId !== userId) {
        s.peer.userId = userId;
  ...
  // ルーム接続時に同じルームの他ピアのPeerContextを生成するヘルパー
